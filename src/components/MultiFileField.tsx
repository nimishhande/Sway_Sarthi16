    ...
    <div>
      ...
      <div className="mt-1.5 space-y-2">
        {values.map((path) => (
          <div
            key={path}
            className="flex items-center justify-between gap-3 rounded-lg border border-success/40 bg-success/5 px-3 py-2.5"
          >
            <div className="flex items-center gap-2 text-sm min-w-0">
              <CheckCircle2 className="h-4 w-4 text-success shrink-0" />
              <span className="truncate">{fileNames[path] || "Uploaded"}</span>
            </div>
            <button
              type="button"
              onClick={() => remove(path)}
