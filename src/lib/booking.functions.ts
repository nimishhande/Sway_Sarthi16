...
const submitSchema = z.object({
  ...
  aadhaar_front_url: z.string().min(1),
  aadhaar_back_url: z.string().min(1),
  pan_url: z.string().min(1),
  driving_licence_front_url: z.string().min(1),
  driving_licence_back_url: z.string().min(1),
  ...
  .handler(async ({ data }) => {
    ...
    const { data: row, error } = await supabaseAdmin
      ...
      .insert({
        ...
        driving_licence_front_url: data.driving_licence_front_url,
        driving_licence_back_url: data.driving_licence_back_url,
